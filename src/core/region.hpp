...
class PendingRegion: public QObject {
	...
	// clang-format on
	/// A list of items whose geometries define sub-regions of this region.
	///
	/// Unlike @@regions, which requires explicit `Region` children, `items`
	/// accepts a JavaScript array of items and auto-generates regions for each
	/// one using @@itemRadius, @@itemShape, and @@itemIntersection.
	///
	/// This property can be bound to a dynamic expression, making it suitable
	/// for cases where the number of tracked items changes at runtime.
	///
	/// Null or non-item entries are ignored.
	/// ```qml
	/// Region {
	///   items: [toast1, toast2, toast3]
	///   itemRadius: 10
	/// }
	/// ```
	Q_PROPERTY(QVariantList items READ items WRITE setItems NOTIFY itemsChanged);
	/// Corner radius for regions generated by @@items. Defaults to 0.
	/// All four corners use the same radius (uniform rounding).
	Q_PROPERTY(qint32 itemRadius MEMBER mItemRadius NOTIFY itemRadiusChanged);
	/// Shape for regions generated by @@items. Defaults to `Rect`.
	Q_PROPERTY(RegionShape::Enum itemShape MEMBER mItemShape NOTIFY itemShapeChanged);
	/// Intersection strategy for regions generated by @@items. Defaults to `Combine`.
	Q_PROPERTY(Intersection::Enum itemIntersection MEMBER mItemIntersection NOTIFY itemIntersectionChanged);
...
signals:
	...
	void itemShapeChanged();
	void itemIntersectionChanged();
